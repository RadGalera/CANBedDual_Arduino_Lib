...
void CANBedDual::send(unsigned long id, unsigned char ext, unsigned char rtr, unsigned char fd, unsigned char len, unsigned char *dta)
{
    ...
    long2char(id, &dsend[1]);    // writes id big-endian into dsend[1..4]
...
byte CANBedDual::read(unsigned long *id, int *ext, int *rtr, int *fd, int *len, unsigned char *str)
{
    ...
    unsigned long _id = char2long(&strg[0]);
    *id  = _id;
    *ext = strg[4];
    *rtr = strg[5];
    *fd  = strg[6];
    ...
    *len = dtaLen;
    ...
    if(dtaLen <= 32)
    {
        // Fix for upstream issue #6: request only as many bytes as the frame
        // actually contains. Original code always requested 8 bytes which left
        // garbage in the I2C buffer for short frames, corrupting subsequent reads.
        CANI2C.beginTransmission(I2C_ADDR);
